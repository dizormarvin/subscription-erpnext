...
class PSOF(Document):
    ...
    def validate_bills(self, programs):
        for program in programs:
            if frappe.db.exists({'doctype': 'PSOF Program Bill', 'subscription_program':  program.subscription_contract,
                                 'psof': program.psof, 'customer_name': program.customer_name}):
                frappe.throw(f'{bill.subscription_program} for PSOF: {bill.psof} already exists')
        return True
        ...
        if self.validate_bills(progs):
            ...
            for i in progs:
                ...
                while start < end:
                    ...
                    doc.active = 1 if i.renewal == 1 else 0
